...
func (c *UsergroupsClient) DisableUsergroups(name string) (res structs.DisableUsergroupsResponse, err error) {

	//Build request
	reqBod := disableStruct{
		Token: c.token,
		Name:  string,
	}
	err = jsonRequest(usergroupsURL, "disable", c.token, reqBod, &res)
	check(err)
	//Return Response
	return
}
